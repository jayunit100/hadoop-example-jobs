...
public class MultiFileWordCount extends Configured implements Tool {
	...
	public static class MultiFileLineRecordReader implements
		...
		static final Logger log = LoggerFactory
				.getLogger(MultiFileWordCount.class);
		...
		public MultiFileLineRecordReader(Configuration conf,
			...
			/**
			 * 
			 @Override public FSDataInputStream open(Path f, int bufferSize)
			 *           throws IOException { if (!exists(f)) { throw new
			 *           FileNotFoundException(f.toString()); } return new
			 *           FSDataInputStream( new BufferedFSInputStream( new
			 *           LocalFSFileInputStream(f), bufferSize)); }
			 */
			currentStream = fs.open(file);
			if (currentStream == null)
				throw new RuntimeException("!!! current stream null !!! ");
			currentReader = new BufferedReaderDebuggable(
					new InputStreamReaderDebuggable(currentStream));
		}
		...
		public long getPos() throws IOException {
			/**
			 * log.info("getPos()"); //seems like this is called sometimes even
			 * if //the stream is closed. try{ //this triggers NPE if stream is
			 * closed. currentStream.available();
			 * 
			 * } catch(Throwable t){ t.printStackTrace(); throw new
			 * RuntimeException("Failing: current stream ..." + t.getMessage());
			 * }
			 **/
			log.info("!!!--- getting position of ::: currentStream ---!!! "
					+ currentStream);
		...
		public boolean next(WordOffset key, Text value) throws IOException {
			log.info(next++ + " next : " + next++ + " currentstream="
					+ currentStream);
			...
			do {
				...
				if (line == null) {
					// close the file
					log.info(next + " closing stream line count=" + count
							+ " splitstotal= " + split.getNumPaths());
					currentReader.close();
					...
					log.info(next + " opening new stream " + file + " "
							+ paths[count].getName());
					currentStream = fs.open(file);
